...
import { BookRideDto } from '../rides/interfaces/interfaces_ride.interface'; // Corrected import path
...
export class UsersController {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  @ApiResponse({ status: 400, description: 'Bad request.' })
  async createBookingRequest(@Body() bookRideDto: BookRideDto, @Request() req) {
    try {
      this.logger.log(`Request user: ${JSON.stringify(req.user)}`);
      const userId = req.user?.id;
      if (!userId) {
      ...
      this.logger.log(`Creating booking request for user ID: ${userId}, ride ID: ${bookRideDto.rideId}`);
      return await this.usersService.createBookingRequest(bookRideDto.rideId, userId, bookRideDto.passengerCount);
    } catch (error) {

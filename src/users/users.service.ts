...
export class UsersService {
  ...
  async createBookingRequest(rideId: string, passengerId: string, passengerCount: number = 1) {
    ...
    if (!Number.isInteger(passengerCount) || passengerCount < 1) {
      this.logger.error(`Invalid passenger count: ${passengerCount}. Must be a positive integer.`);
      throw new BadRequestException('Passenger count must be a positive integer');
    }
    try {
      this.logger.log(`Creating booking request for rideId: ${rideId}, passengerId: ${passengerId}, passengerCount: ${passengerCount}`);
      return await this.prisma.$transaction(async (prisma) => {
        ...
        if (ride.driverId === passengerId) {
          this.logger.warn(`Passenger ${passengerId} cannot book their own ride ${rideId}`);
          throw new BadRequestException('You cannot book your own ride');
        }

        if (ride.status !== 'active') {
          this.logger.warn(`Ride ${rideId} is not available for booking (status: ${ride.status})`);
          throw new BadRequestException('Ride is not available for booking');
        }

        if (ride.availableSeats < passengerCount) {
          this.logger.warn(`Not enough available seats for ride ${rideId}. Requested: ${passengerCount}, Available: ${ride.availableSeats}`);
        ...
        this.logger.log(`Booking request created for rideId: ${rideId}, passengerId: ${passengerId}, passengerCount: ${passengerCount}`);
        return {
          success: true,
          bookingRequest: {
  ...
  async acceptBookingRequest(bookingRequestId: string, driverId: string) {
    ...
    try {
      ...
      return await this.prisma.$transaction(async (prisma) => {
        ...
        if (bookingRequest.ride.availableSeats < bookingRequest.passengerCount) {
          this.logger.warn(`Not enough available seats for ride ${bookingRequest.rideId}`);
        ...
        const newAvailableSeats = bookingRequest.ride.availableSeats - bookingRequest.passengerCount;
